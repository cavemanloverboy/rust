...
use rustc_errors::{DiagCtxt, ErrorGuaranteed, IntoDiagnostic, MultiSpan};
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn emit_node_span_lint(
        ...
        into_diag: impl IntoDiagnostic<'tcx, ()>,
    ) {
        let (level, src) = self.lint_level_at_node(lint, hir_id);
        lint_level(self.sess, lint, level, src, Some(span.into()), into_diag)
    }
    ...
    /// Emit a lint from a diagnostic struct (some type that implements `IntoDiagnostic`).
    #[track_caller]
    pub fn emit_node_lint(
        ...
        self.node_lint(lint, id, into_diag)
    }
    ...
    pub fn node_lint(
        ...
        lint_level(self.sess, lint, level, src, None, into_diag);
    }
