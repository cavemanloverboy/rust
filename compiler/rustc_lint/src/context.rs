...
use rustc_errors::{Diag, DiagMessage, IntoDiagnostic, MultiSpan};
...
pub trait LintContext {
    ...
    /// [`lint_level`]: rustc_middle::lint::lint_level#decorate-signature
    #[rustc_lint_diagnostics]
    fn opt_span_lint<'a, S: Into<MultiSpan>>(
        &self,
        lint: &'static Lint,
        span: Option<S>,
        into_diag: impl IntoDiagnostic<'a, ()>,
    );

    /// Emit a lint at `span` from a diagnostic struct (some type that implements `IntoDiagnostic`).
    fn emit_span_lint<'a, S: Into<MultiSpan>>(
        &self,
        lint: &'static Lint,
        span: S,
        into_diag: impl IntoDiagnostic<'a, ()>,
    ) {
        self.opt_span_lint(lint, Some(span), into_diag);
    }
    ...
    /// [`lint_level`]: rustc_middle::lint::lint_level#decorate-signature
    #[rustc_lint_diagnostics]
    fn span_lint<'a, S: Into<MultiSpan>>(
        &self,
        lint: &'static Lint,
        span: S,
        into_diag: impl IntoDiagnostic<'a, ()>,
    ...
    /// Emit a lint from a lint struct (some type that implements `IntoDiagnostic`).
    fn emit_lint<'a>(&self, lint: &'static Lint, into_diag: impl IntoDiagnostic<'a, ()>) {
        self.opt_span_lint(lint, None as Option<Span>, into_diag);
    }
    ...
    /// [`lint_level`]: rustc_middle::lint::lint_level#decorate-signature
    #[rustc_lint_diagnostics]
    fn lint<'a>(&self, lint: &'static Lint, into_diag: impl IntoDiagnostic<'a, ()>) {
        self.opt_span_lint(lint, None as Option<Span>, into_diag);
    }
